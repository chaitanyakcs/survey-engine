...
def seed_qnr_labels(db: Session) -> int:
    ...
    for category, labels in QNR_LABELS_DATA.items():
        ...
        for label_data in labels:
            # Convert applicable_labels list to PostgreSQL array format
            applicable_labels_str = '{' + ','.join(f'"{label}"' for label in label_data['applicable_labels']) + '}' if label_data['applicable_labels'] else '{}'
            ...
            db.execute(
                text("""
                    ...
                    VALUES (:name, :category, :description, :mandatory, :label_type,
                            ARRAY[:applicable_labels], :section_id, :display_order)
