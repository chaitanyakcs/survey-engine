...
class AIAnnotationService:
    ...
    async def create_annotations_from_evaluation(
        ...
        """
        try:
            logger.info(f"🤖 Updating AI annotations for survey {survey_id}")
            
            # Don't delete existing annotations - update them instead
            # This preserves labels from LabelDetectionNode and other sources
            ...
            # Update or create question annotations
            for q_annotation in evaluation_result.question_annotations:
                try:
                    result = await self._update_or_create_question_annotation(
                        q_annotation, survey_id, annotator_id
                    )
                    created_annotations["question_annotations"].append(result['annotation_id'])
                    if result['created']:
                        created_annotations["total_created"] += 1
                    else:
                        created_annotations["total_updated"] += 1
                    
                except Exception as e:
                    error_msg = f"Failed to update/create question annotation for {q_annotation.get('question_id', 'unknown')}: {str(e)}"
                    logger.error(f"❌ {error_msg}")
                    created_annotations["errors"].append(error_msg)
            
            # Update or create section annotations
            for s_annotation in evaluation_result.section_annotations:
                try:
                    result = await self._update_or_create_section_annotation(
                        s_annotation, survey_id, annotator_id
                    )
                    created_annotations["section_annotations"].append(result['annotation_id'])
                    if result['created']:
                        created_annotations["total_created"] += 1
                    else:
                        created_annotations["total_updated"] += 1
                    ...
                    error_msg = f"Failed to update/create section annotation for {s_annotation.get('section_id', 'unknown')}: {str(e)}"
                    logger.error(f"❌ {error_msg}")
                    created_annotations["errors"].append(error_msg)
            ...
            logger.info(f"✅ Updated/created {created_annotations['total_created']} new and {created_annotations['total_updated']} existing AI annotations for survey {survey_id}")
            
            return created_annotations
            
        except Exception as e:
            logger.error(f"❌ Error updating AI annotations: {str(e)}")
            self.db_session.rollback()
            raise
    
    async def _update_or_create_question_annotation(
        self, 
        q_data: Dict[str, Any], 
        survey_id: str, 
        annotator_id: str
    ) -> Dict[str, Any]:
        """Update existing question annotation or create new one, preserving labels"""
        ...
        # Check if annotation exists (could be from LabelDetectionNode)
        existing_annotation = self.db_session.query(QuestionAnnotation).filter(
            QuestionAnnotation.question_id == unique_question_id,
            QuestionAnnotation.survey_id == survey_id
        ).first()
        
        if existing_annotation:
            # Update existing annotation, preserving labels
            logger.info(f"🔄 Updating existing question annotation for {q_data.get('question_id')}")
            
            # Preserve existing labels - don't override with empty dict
            existing_labels = existing_annotation.labels
            if not existing_labels:
                existing_labels = []
            
            # Update AI evaluation fields but keep labels and removed_labels
            existing_annotation.quality = q_data.get("quality", 3)
            existing_annotation.relevant = q_data.get("relevant", 3)
            existing_annotation.methodological_rigor = q_data.get("methodological_rigor", 3)
            existing_annotation.content_validity = q_data.get("content_validity", 3)
            existing_annotation.respondent_experience = q_data.get("respondent_experience", 3)
            existing_annotation.analytical_value = q_data.get("analytical_value", 3)
            existing_annotation.business_impact = q_data.get("business_impact", 3)
            existing_annotation.comment = q_data.get("comment", "")
            
            # Only update AI fields if not already set
            if existing_annotation.annotator_id != "ai_system":
                existing_annotation.ai_generated = True
                existing_annotation.ai_confidence = q_data.get("ai_confidence", 0.8)
                existing_annotation.generation_timestamp = datetime.now()
            ...
            logger.info(f"✅ Creating new question annotation for {q_data.get('question_id')}")
            annotation = QuestionAnnotation(
                ...
                labels=[],  # Empty labels - will be filled by LabelDetectionNode or user
                removed_labels=[],
                ai_generated=True,
    ...
    async def _update_or_create_section_annotation(
        self, 
        s_data: Dict[str, Any], 
        survey_id: str, 
        annotator_id: str
    ) -> Dict[str, Any]:
        """Update existing section annotation or create new one, preserving labels"""
        ...
        existing_annotation = self.db_session.query(SectionAnnotation).filter(
            SectionAnnotation.section_id == unique_section_id,
        ...
        if existing_annotation:
            ...
            # Update AI evaluation fields but keep labels
            existing_annotation.quality = s_data.get("quality", 3)
            existing_annotation.relevant = s_data.get("relevant", 3)
            existing_annotation.methodological_rigor = s_data.get("methodological_rigor", 3)
            existing_annotation.content_validity = s_data.get("content_validity", 3)
            existing_annotation.respondent_experience = s_data.get("respondent_experience", 3)
            existing_annotation.analytical_value = s_data.get("analytical_value", 3)
            existing_annotation.business_impact = s_data.get("business_impact", 3)
            existing_annotation.comment = s_data.get("comment", "")
            
            # Only update AI fields if not already set
            if existing_annotation.annotator_id != "ai_system":
                existing_annotation.ai_generated = True
                existing_annotation.ai_confidence = s_data.get("ai_confidence", 0.8)
                existing_annotation.generation_timestamp = datetime.now()
            ...
            # Create new annotation
            logger.info(f"✅ Creating new section annotation for {s_data.get('section_id')}")
